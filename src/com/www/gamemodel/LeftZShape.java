package com.www.gamemodel;
	 ...
	 *
	 */
public class LeftZShape extends BaseShape {
	/**
	 * 左Z形方块构造方法，传递下落区域对象
	 * @param GridBlock
	 */
	...
	public void right() {
		...
		if (canMove) {
			switch (whichTime) {
			case 0:	//形态0的右移条件和移动
				if ((shapeX[3]!=GridBlock.COL)&&(GridBlock.xyList.get(shapeXYToInt((shapeX[1]+1), shapeY[1])).isDraw()==false)&&
						(GridBlock.xyList.get(shapeXYToInt((shapeX[3]+1), shapeY[3])).isDraw()==false)) {
					shapeX[0]+=1;shapeX[1]+=1;shapeX[2]+=1;shapeX[3]+=1;
				}
				break;
	...
	private void oneToTwo(){
		if ((shapeX[2]!=1)&&(GridBlock.xyList.get(shapeXYToInt((shapeX[0]-1), shapeY[0])).isDraw()==false)&&
				(GridBlock.xyList.get(shapeXYToInt((shapeX[0]-2), shapeY[0])).isDraw()==false)&&
				(GridBlock.xyList.get(shapeXYToInt(shapeX[1], (shapeY[1]+1))).isDraw()==false)) {	//旋转条件
			whichTime=2;
			shapeX[0]-=2;
			shapeX[1]-=1;shapeY[1]-=1;
			shapeX[3]+=1;shapeY[3]-=1;
